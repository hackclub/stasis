...
export async function GET(
  ...
  if (!parsed) {
    checks.push({ key: 'checks_01_github_valid', label: 'GitHub repo valid', passed: false, detail: githubRepo ? 'Could not parse GitHub URL' : 'No GitHub repo URL' });
    // All other checks fail if no valid repo
    checks.push({ key: 'checks_02_readme_exists', label: 'README exists', passed: false, detail: 'No valid repo' });
    checks.push({ key: 'checks_03_readme_has_photo', label: 'README has photo', passed: false, detail: 'No valid repo' });
    checks.push({ key: 'checks_05_3d_file', label: '3D model file', passed: false, detail: 'No valid repo' });
    checks.push({ key: 'checks_06_3d_source', label: '3D source file (F3D/STEP)', passed: false, detail: 'No valid repo' });
    checks.push({ key: 'checks_07_firmware_file', label: 'Firmware file', passed: false, detail: 'No valid repo' });
    checks.push({ key: 'checks_08_bom_file', label: 'BOM file', passed: false, detail: 'No valid repo' });
  ...
  checks.push({
    ...
    passed: repoValid,
    detail: repoValid ? `${owner}/${repo}` : 'Repo not found or not accessible',
  });
