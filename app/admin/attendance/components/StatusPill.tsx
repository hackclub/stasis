...
export function StatusPill({ status, size = 'sm' }: Readonly<{ status: AttendanceStatus; size?: 'sm' | 'md' }>) {
  ...
  return (
    <span className={`inline-flex items-center uppercase tracking-widest font-medium border ${sizeCls} ${cls}`}>
      {STATUS_LABEL[status]}
    </span>
  );
}
...
export function FlagPill({
  ...
  return (
    <span title={title} className={`inline-flex items-center uppercase tracking-widest font-medium text-[11px] px-2 py-0.5 border ${cls}`}>
      {label}
    </span>
  );
}
