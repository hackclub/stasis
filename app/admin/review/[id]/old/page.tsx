...
export default function ReviewDetailPage() {
  ...
  useEffect(() => {
    ...
    function prime(targetId: string) {
      ...
      router.prefetch(`/admin/review/${targetId}/old${filterQS}`);
    }
  ...
  async function submitReview(result: string, overrides?: {
    ...
    if (data.navigation.nextId) {
      router.push(`/admin/review/${data.navigation.nextId}/old${filterQS}`);
    } else {
      router.push('/admin/review');
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/admin/review/${originalId}/old${filterQS}`);
      }
    } catch (err) {
  ...
  async function skipToNext() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (next) {
          router.push(`/admin/review/${next.id}/old${filterQS}`);
          return;
        }
      }
    } catch {}
  ...
  const detailHotkeys: HotkeyBinding[] = [
    ...
    { key: 'j', description: 'Next submission', group: 'Navigation', handler: () => { if (data?.navigation.nextId) router.push(`/admin/review/${data.navigation.nextId}/old${filterQS}`); } },
    { key: 'k', description: 'Previous submission', group: 'Navigation', handler: () => { if (data?.navigation.prevId) router.push(`/admin/review/${data.navigation.prevId}/old${filterQS}`); } },
