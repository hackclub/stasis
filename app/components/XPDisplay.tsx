...
export function XPDisplay() {
  ...
  return (
    <div className="bg-cream-100 border-2 border-cream-400 p-4">
      <div className="mb-4">
        <h2 className="text-orange-500 text-lg uppercase tracking-wide">Your XP</h2>
        <p className="text-brown-800 text-sm">Work on your projects and log a journal entry every day to earn XP and increase your multiplier. Get prizes like a Bandana, T-Shirt, and Hoodie!</p>
      </div>

      <div className="flex items-baseline gap-2 mb-4">
        <p className="text-brown-800 text-4xl font-bold">{data.totalXP.toLocaleString()}</p>
      ...
      <div className="flex flex-col lg:flex-row lg:items-start gap-4 lg:gap-8 mb-4">
        <div className="flex gap-4 sm:gap-6 flex-wrap">
          <div>
            ...
            <p className="text-brown-800 text-lg sm:text-xl">{data.dayStreak} day{data.dayStreak !== 1 ? 's' : ''}</p>
          </div>
          <div>
            <p className="text-cream-600 text-xs uppercase">Week Streak</p>
            <p className="text-brown-800 text-lg sm:text-xl">{data.weekStreak} week{data.weekStreak !== 1 ? 's' : ''}</p>
          </div>
          <div className="bg-orange-500/20 border border-orange-500 px-3 sm:px-4 py-2 -my-2">
            <p className="text-orange-400 text-xs uppercase">Multiplier</p>
            <p className="text-orange-400 text-xl sm:text-2xl font-bold">{data.multiplier}x</p>
          </div>
        </div>
        ...
        <div className="flex flex-col overflow-x-auto">
                    ...
                    <div 
                      className={`px-2 py-1 flex items-center justify-center border-2 transition-all ${
                        unlocked 
                          ...
                          : isNext
                          ? 'bg-orange-500/20 border-orange-500 border-dashed'
                          : 'bg-cream-200 border-cream-400'
                      }`}
                      title={`${prize.xpRequired} XP`}
                    >
                      <span className={`text-[10px] uppercase tracking-wide font-medium whitespace-nowrap ${unlocked ? 'text-white' : isNext ? 'text-orange-500' : 'text-cream-600'}`}>{prize.name}</span>
                    </div>
                    <p className={`text-[10px] mt-0.5 ${unlocked ? 'text-orange-500' : 'text-cream-500'}`}>
                      {prize.xpRequired}
                    </p>
                  </div>
                  {i < PRIZES.length - 1 && (
          ...
          <p className={`text-xs mt-1 ${nextPrize ? 'text-cream-600' : 'text-orange-500'}`}>
            {nextPrize ? <><span className="text-sm font-medium text-brown-800">{nextPrize.xpRequired - data.totalXP}</span> XP to {nextPrize.name}</> : '✓ All unlocked'}
          </p>
        </div>
      </div>

      {/* Progress bar - solid orange with dotted line markers for incentives */}
      <div className="relative h-8 bg-orange-500 overflow-visible">
        ...
        {PRIZES.map((prize, index) => {
          ...
          return (
            ...
            >
              ...
              <span className={`absolute -bottom-5 text-[10px] font-medium whitespace-nowrap ${unlocked ? 'text-orange-500' : 'text-cream-600'} ${isLast ? 'right-0' : ''}`}>
