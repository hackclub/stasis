...
interface WorkSession {
  id: string
  type: 'JOURNAL' | 'TIMELAPSE' | 'SHORT_VIDEO_EXPLANATION'
  hoursClaimed: number
                      ...
                      <div className="flex-1 overflow-y-auto mt-4">
                        ...
                        {selectedProject.workSessions.length > 0 ? (
                          <div className="space-y-2">
                            {selectedProject.workSessions.map((session) => (
                              <div 
                                key={session.id}
                                className="bg-cream-950 p-3 border border-cream-800"
                              >
                                <div className="flex items-center justify-between mb-1">
                                  <span className="text-xs bg-cream-850 text-cream-500 px-2 py-0.5 uppercase">
                                    {session.type.replace('_', ' ')}
                                  </span>
                                  <div className="text-right">
                                    <span className="text-cream-100 text-sm">{session.hoursClaimed}h claimed</span>
                                    {session.hoursApproved !== null && (
                                      <span className="text-brand-500 text-sm ml-2">({session.hoursApproved}h approved)</span>
                                    )}
                                  </div>
                                </div>
                                {session.content && (
                                  <p className="text-cream-500 text-xs mt-2 line-clamp-2">{session.content}</p>
                                )}
